...
/**
 * Preferred display side for each muscle group. Used by view="auto" to pick
 * which side of the body silhouette is most representative for the muscle.
 */
export const MUSCLE_VIEW: Record<MuscleGroup, BodyView> = {
  [MuscleGroup.Chest]: 'anterior',
  ...
  [MuscleGroup.FullBody]: 'anterior',
};

/** Human-readable labels (also used for aria descriptions). */
...
/**
 * Static per-muscle-group color tokens, mirrored as CSS custom properties in styles.scss.
 * Used as the default fill for highlighted regions.
 */
export const MUSCLE_COLOR_VAR: Record<MuscleGroup, string> = {
  [MuscleGroup.Chest]: 'var(--muscle-chest)',
  ...
  [MuscleGroup.Core]: 'var(--muscle-core)',
  [MuscleGroup.FullBody]: 'var(--muscle-full-body)',
};

export function viewPolygons(view: BodyView): PolygonData[] {
  return view === 'anterior' ? ANTERIOR : POSTERIOR;
}

/**
 * Tight viewBox per muscle group, used by `focus="true"`. Crops the SVG so the
 * highlighted region fills the visible area — useful when the icon is small
 * (e.g. per-exercise rows) and a full-body silhouette would be unreadable.
 *
 * Format matches SVG `viewBox`: "minX minY width height" within the 0 0 100 200 canvas.
 */
export const MUSCLE_FOCUS_VIEWBOX: Record<MuscleGroup, string> = {
  [MuscleGroup.Chest]: '18 25 64 40',
  [MuscleGroup.Back]: '15 15 70 95',
  [MuscleGroup.Shoulders]: '12 28 76 32',
  [MuscleGroup.Biceps]: '10 40 80 40',
...
export const FULL_VIEWBOX = '0 0 100 200';
